...
private:
    ...
    std::vector<std::regex> file_masks;
...
/*        bool answ = false;
    ...
    void print() {
        /*                   file_list.erase(std::remove_if(file_list.begin(), file_list.end(), [](auto &x) { return !x.duplicate; }),
                                   file_list.end());*/
        std::sort(file_list.begin(), file_list.end());
        for (int i = 0; i < file_list.size(); ++i) {
            const auto &f = file_list[i];
            if (!f.duplicate) break;
            if (i > 0 && f.hash[0] != file_list[i - 1].hash[0])
                std::cout << '\n';
